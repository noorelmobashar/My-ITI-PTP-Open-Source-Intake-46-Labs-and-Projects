...
<template>
  <div class="card card-side overflow-hidden border border-white/10 bg-white/8 shadow-[0_20px_80px_rgba(15,23,42,0.55)] backdrop-blur-xl flex-col md:flex-row">
    ...
    <div class="card-body gap-5 md:w-1/2 md:p-8">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <span v-if="!props.product.isAvailable" class="badge badge-error border-0 px-4 py-3 font-bold shadow-lg">Out of Stock</span>
      </div>

      <p class="max-w-xl text-base leading-7 text-slate-300">{{ props.product.description }}</p>
      ...
      <div class="mt-2 rounded-3xl border border-white/10 bg-slate-900/60 p-5">
        ...
        <span v-if="props.product.discount > 0" class="mr-3 text-lg text-slate-500 line-through">
          ${{ props.product.price }}
        </span>
        <span
          class="text-4xl font-black tracking-tight"
          :style="{ color: props.product.discount > 0 ? '#16a34a' : '' }"
        >
          ${{ discountedPrice }}
        </span>
        <span v-if="props.product.discount > 0" class="ml-3 text-sm font-semibold text-success">
          ({{ props.product.discount }}% off)
        </span>
        <p class="mt-3 text-sm font-medium text-slate-300">
          Stock: <span class="font-bold text-white">{{ props.product.stock }}</span> units available
        </p>
      </div>
      ...
      <div class="card-actions mt-2 items-center justify-between gap-4">
        ...
        <button
          ...
          @click="$emit('decrease-stock', props.product.id)"
        >
          {{ props.product.isAvailable ? 'Buy Now' : 'Out of Stock' }}
        </button>
      </div>
